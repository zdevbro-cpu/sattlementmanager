  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-4">
        <SummaryCard label="전체 신청서" value={`${sum.total}건`} tint="#e0edff" fg="#2563eb" icon={BookOpen} />
        <SummaryCard label="오늘 접수된 신청서" value={`${sum.todayCount}건`} tint="#e2f7ec" fg="#16a34a" icon={CalendarCheck} />
        <SummaryCard label="수기신청서 사진 첨부" value={`${sum.withPhoto}건`} tint="#fff1e0" fg="#f59e0b" icon={Camera} />
        <SummaryCard label="양식 PDF 생성" value={`${sum.withPdf}건`} tint="#f3e8ff" fg="#7c3aed" icon={FileText} />
      </div>
              ...
              <div className="space-y-2">
                ...
                {a.drivePdfFileId ? (
                  ...
                  >
                    ...
                    <span className="inline-flex items-center gap-1 text-primary font-bold">PDF 열기 <ExternalLink size={12} /></span>
                  </a>
                ) : (
                  <button
                    onClick={onGeneratePdf}
                    disabled={pdfBusy}
                    className="inline-flex h-10 w-full items-center justify-center gap-1.5 rounded-[8px] bg-primary text-[13px] font-bold text-white hover:brightness-110 disabled:opacity-60"
                  >
                    {pdfBusy ? 'PDF 생성 중…' : (<><FileText size={15} /> 입력정보로 양식 PDF 생성</>)}
                  </button>
                )}
